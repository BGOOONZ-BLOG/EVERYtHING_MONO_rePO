$(() => {
  ...
  $("#letter-e a").click((e) => {
    ...
    $.get("notfound", requestData, (data) => {
      ...
      $("#dictionary").html(`An error occurred:
            ${xhr.status}
            ${xhr.responseText}
          `);
    });
  ...
  $("#letter-f form").submit((e) => {
    e.preventDefault();

    $.post($(e.target).attr("action"), $(e.target).serialize(), (data) => {
      $("#dictionary").html(data);
    });
  });
  ...
  $("body").on("click", "h3.term", (e) => {
    $(e.target).siblings(".definition").slideToggle();
  });
});

Promise.all([$.get("a.html"), jQuery.ready]).then(([content]) => {
